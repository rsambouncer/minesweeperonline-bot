...
function autoauto(a1,b1,level,run){
    //I slowed down the bot so the user can see what the bot is doing
	setTimeout(function(){loop(a1,b1,level,run);}, 20);
    //loop(a1,b1,level2,run,run2); //use this line for no delay
}

function loop(a1,b1,level,run){
	if(level==1){
		for(let a=a1;a<=16;a++){ for(let b=1;b<=30;b++){
        	if(autoLevel1(a,b)){ autoauto(a,b,1,true); return;}
    	}}
        if(run) autoauto(1,1,1,false); 
        else autoauto(1,1,2,false); 
    }
    else if(level==2){
        for(let a=a1;a<=16;a++) for(let b=1;b<=30;b++){
            if(autoLevel2(a,b,0)||autoLevel2(a,b,1)){ autoauto(a,b,2,true); return;}
        }
        if(run) autoauto(1,1,1,false);
    	...
    	if(face!=="facewin"&&face!=="facedead"){
          ...
          for(let a=1;a<=16;a++) for(let b=1;b<=30;b++){
              let el = square(a,b);
              if(el.classList[1]==="blank") blanks.push(el);
          }
          leftClick(blanks[Math.floor(Math.random()*blanks.length)]);
          let face = document.getElementById("face").classList[0];
          if(face!=="facewin"&&face!=="facedead") autoauto(1,1,1,false);
        }
    }
}

document.addEventListener("keydown",function(e){ if(e.keyCode==67) autoauto(1,1,1,false);});
